...
OAT.RDFStore = function(tripleChangeCallback, optObj) {
    ...
    this.addURL = function(url, optObj) {
        ...
        var rdfstore_addurl_cb = function(str) {
                ...
                for (var i = 0; i < triples.length; i++) {
                    var t = triples[i]; /* remove all scripts to prevent their execution */
                    t[2] = sanitize(t[2]);
                ...
                OAT.MSG.send(self, "STORE_LOADED", {
                    url: url
                });
            }
        ...
        opt.ajaxOpts.onerror = function(xhr) {
            ...
            OAT.MSG.send(self, "STORE_LOAD_FAILED", {
                url: url,
                xhr: xhr
            });
        }
        ...
        OAT.MSG.send(this, "STORE_LOADING", {
            url: url
        });
    ...
    this.addXmlDoc = function(xmlDoc, href) {
        ...
        var triples = OAT.RDF.toTriples(xmlDoc); /* sanitize triples */
        for (var i = 0; i < triples.length; i++) {
            var t = triples[i];
            t[2] = t[2].replace(/<script[^>]*>/gi, '');
        }
        self.addTriples(triples, href);
        OAT.MSG.send(this, "STORE_LOADED", {
            url: xmlDoc.baseURI
        });
    }
    ...
    this.remove = function(url) {
        ...
        self.rebuild(true);
        OAT.MSG.send(self, "STORE_REMOVED", {
            url: url
        });
    }
    ...
    this.enable = function(url) {
        ...
        self.rebuild(true);
        OAT.MSG.send(self, "STORE_ENABLED", {
            url: url
        });
    }
    ...
    this.disable = function(url) {
        ...
        self.rebuild(true);
        OAT.MSG.send(self, "STORE_DISABLED", {
            url: url
        });
    }
            ...
            for (var j in preds) {
                ...
                for (var k = 0; k < pred.length; k++) {
                    ...
                    if (value in conversionTable) {
                        ...
                        pred[k] = target;
                        if (target.back.indexOf(item) == -1) {
                            target.back.push(item);
                        }
                    }
                }
            } /* predicates */
    ...
    this.applyFilters = function(type, hardReset) {
        ...
        switch (type) {
        case OAT.RDFStoreData.FILTER_ALL:
            /* all filters */
    ...
    this.getContentType = function(str) { /* 0 - generic, 1 - link, 2 - mail, 3 - image */
        if (str.match(/^http.*(jpe?g|png|gif)(#[^#]*)?$/i)) {
            return 3;
        }
