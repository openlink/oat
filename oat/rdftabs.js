...
OAT.RDFTabsData = {
    MARKER_MODE_DISTINCT_O: 1,
    // Old default behaviour - distinct markers by ouri
    MARKER_MODE_BY_TYPE: 2,
    // Markers by item type match
    MARKER_MODE_EXPLICIT: 3,
    // Markers by explicit property oat:rdfTabsMarker <marker URL>
    MARKER_MODE_AUTO: 4
};
...
OAT.RDFTabs.parent = function(obj) { /* methods & properties that need to be implemented by each RDFTab */
...
OAT.RDFTabs.browser = function(parent, optObj) {
    ...
    this.dataDiv = OAT.Dom.create("div", {
        className: "rdf_data"
    });
    this.sortDiv = OAT.Dom.create("div", {
        className: "rdf_sort"
    });
    ...
    this.sort = function(predicate) {
        var sf = function(a, b) { /* find values of this predicate */
                var a_ = false;
                var b_ = false;
                var ap = a.preds;
                var bp = b.preds;
    ...
    this.drawItem = function(item) { /* one item */
        var s_ctr = OAT.Dom.create("div", {
            className: "rdf_item"
        });
        var h = OAT.Dom.create("h3", {
            className: "rdf_subject"
        });
        ...
        var preds_ctr = OAT.Dom.create("div", {
            className: "rdf_preds"
        });
        ...
        for (var p in preds) {
            ...
            if (!ok) {
                continue;
            } /* don't draw this property */

            var p_ctr = OAT.Dom.create("div", {
                className: "rdf_p"
            });
            ...
            var pred_ul = OAT.Dom.create("ul", {
                className: "rdf_o"
            })
            for (var i = 0; i < pred.length; i++) {
                var pred_li = OAT.Dom.create("li", {
                    className: "rdf_o"
                });
                var value = pred[i];
    ...
    this.drawData = function() { /* set of items */
        ...
        var createGroup = function(label) {
                if (groupDiv) {
                    groupDiv.appendChild(OAT.Dom.create("div", {
                        className: "rdf_clear"
                    }));
                }
                groupDiv = OAT.Dom.create("div", {
                    display: "none",
                    className: "rdf_group"
                });
                groupDiv.state = 0;
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if (self.groupMode) { /* grouping */
                var gv = findGV(item);
                if (gv != groupValue || i == 0) { /* create new group */
                    groupValue = gv;
                    createGroup(gv);
                }
    ...
    this.drawPager = function() {
        ...
        var div = OAT.Dom.create("div", {
            className: "pgr_page_no"
        });
        var gd = OAT.Dom.create("div", {
            className: "pgr_grouping"
        });
        ...
        cnt.innerHTML = "" + count + " records (" + tcount + " triples, " + pcount + " properties) match selected filters. ";
    ...
    this.drawSort = function() {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            for (var p in preds) {
                ...
                for (var j = 0; j < self.parent.store.filtersProperty.length; j++) {
                    if (self.parent.store.filtersProperty[j][0] == p) {
                        index2 = p;
                    }
                }
        ...
        for (var i = 0; i < list.length; i++) {
            ...
            if (i) {
                self.sortDiv.appendChild(OAT.Dom.text(", "));
            }
...
OAT.RDFTabs.navigator = function(parent, optObj) {
    ...
    this.waiting = false;
    this.mlCache = [];
    this.topDiv = OAT.Dom.create("div", {
        className: "rdf_nav"
    });
    ...
    this.attach = function(elm, item) { /* attach navigation to link */
        ...
        OAT.Event.attach(elm, "click", function(event) {
            if (arrow) elm.removeChild(arrow);
            arrow = null; /* disable default onclick event for anchor */
            OAT.Event.prevent(event);
    ...
    this.dattach = function(elm, uri) { /* attach dereference to link */
        OAT.Event.attach(elm, "click", function(event) { /* disable default onclick event for anchor */
            OAT.Event.prevent(event);
            self.waiting = true;
    ...
    this.drawPredicate = function(value) { /* draw one pred's content; return ELM */
        ...
        if (typeof(value) == "object") { /* resource */
            ...
            for (var j = 0; j < items.length; j++) {
                var item = items[j]; /* handle anchors to local file */
            ...
            if (type == 3) { /* image */
                ...
                var anchors = [];
                for (var j = 0; j < anchors_.length; j++) {
                    anchors.push(anchors_[j]);
                }
                for (var j = 0; j < anchors.length; j++) {
                    ...
                    if (!done) {
                        self.dattach(anchor, anchor.href);
                    }
    ...
    this.drawSpotlightType = function(label, data, table) {
        ...
        if (self.options.limit < data.length) {
            var toggletr = OAT.Dom.create("tr", {
                className: "rdf_nav_toggle"
            });
            toggletr.appendChild(OAT.Dom.create("td"));
            trset.push(toggletr);
            var td = OAT.Dom.create("td");
            var toggle = OAT.Dom.create("span", {
                className: "toggler"
            });
    ...
    this.drawSpotlight = function(title, uri, obj) { /* list of resources */
        ...
        var t_elm = OAT.Dom.create("h3", {
            className: "rdf_nav_item_title"
        });
        ...
        if (remain) {
            self.drawSpotlightType("[Is Referenced By]", remain, tbody);
        }
    }
    ...
    this.initTop = function() {
        ...
        OAT.Event.attach(self.nav.prev, "click", function() {
            if (self.historyIndex > 0) {
                self.navigate(self.historyIndex - 1);
            }
        });
...
OAT.RDFTabs.triples = function(parent, optObj) {
    ...
    this.redraw = function() {
        if (!self.initialized) {
            self.initialized = true;
            self.grid = new OAT.Grid(self.gridDiv, {
                autoNumber: true,
                allowHiding: true
            });
        }
        ...
        for (var i = 0; i < triples.length; i++) {
            if (i >= self.currentPage * self.options.pageSize && i < (self.currentPage + 1) * self.options.pageSize) {
                ...
                for (var j = 0; j < triple.length; j++) {
                    var str = triple[j]; /* if j = 0, we are subject, so simplify & attach a++ */
                    if (j == 0 || str.match(/^(http|urn|doi)/i)) {
                        self.patchAnchor(j + 1);
                    } /* if j = 2, we are object, find embedded hrefs and process */
                    if (j == 2 && str.match(/href/)) {
                        self.patchEmbedded(j + 1);
                    }
                }
            } /* if in current page */
        } /* for all triples */
...
OAT.RDFTabs.svg = function(parent, optObj) {
    ...
    if (!self.options.description) self.options.description = "This module displays filtered data as SVG Graph. For performance reasons, the number of used triples is limited to " + self.options.limit + "."
    ...
    this.redraw = function() { /* create better triples */
        var triples = [];
        ...
        self.graphsvg = new OAT.GraphSVG(self.elm, x[0], x[1], {
            vertexSize: [4, 8],
            sidebar: false
        });
...
OAT.RDFTabs.PointList = function(opts) {
    ...
    this.insert = function(p, o) {
        ...
        if (typeof(p[0]) == 'string') p[0] = parseFloat(p[0]);

        if (typeof(p[1]) == 'string') p[1] = parseFloat(p[1]);
    ...
    this._ins_1 = function(p, o, st, en) {
        if (self._k(self._list[st]) == p[0]) return (self.ins_y(p, o, self._list[st][1])) // found existing X, now insert y in ylist
        if (st == en) {
            if (self._k(self._list[st]) > p[0]) self.__ins_new(st, p, o);
            else self.__ins_new(st + 1, p, o);
            return p;
        }
        ...
        if (self._k(self._list[split]) < p[0]) return (self._ins_1(p, o, split + 1, en))
        else return (self._ins_1(p, o, st, split));
    }
    ...
    this.ins_y_1 = function(p, o, lst, st, en) {
        if (self._k(lst[st]) == p[1]) {
            if (!self._uniqueInsert) lst.splice(st, 0, new Array(p[1], o));
            return p;
        }

        if (st == en) {
            if (self._k(lst[st]) > p[1]) lst.splice(st, 0, new Array(p[1], o))
            else lst.splice(st + 1, 0, new Array(p[1], o));
            return p;
        }
        ...
        if (self._k(lst[split]) < p[1]) return (self.ins_y_1(p, o, lst, split + 1, en));
        else return (self.ins_y_1(p, o, lst, 0, split));
    }
...
OAT.RDFTabs.map = function(parent, optObj) {
    ...
    this.options = {
        ...
        markerMode: OAT.RDFTabsData.MARKER_MODE_DISTINCT_O,
    ...
    this.usedBlanknodes = [];
    this.pointList = new OAT.RDFTabs.PointList({
        uniqueInsert: true
    });
    ...
    this.extractCoords = function(preds) {
        ...
        for (var p in preds) {
            var pred = preds[p];
            var simple = self.parent.simplify(p);
            if (self.latProperties.find(simple) != -1) {
                coords[0] = pred[0];
            }
            if (self.lonProperties.find(simple) != -1) {
                coords[1] = pred[0];
            }
    ...
    this.tryItem = function(item) {
        ...
        if (self.pointTypes.find(self.parent.simplify(item.type)) != -1) {
            ...
            for (var p in preds) {
                var pred = preds[p];
                var simple = self.parent.simplify(p);
                if (self.keyProperties.find(simple) != -1) {
                    pointResource = pred[0];
                } /* resource containing geo coordinates */
                if (self.locProperties.find(simple) != -1) {
                    locValue = pred[0];
                } /* resource containing geo coordinates */
            }

            if (!pointResource && !locValue) {
                return;
            }
            ...
            if (typeof(pointResource) != "object") {
                return;
            } /* not a reference */
    ...
    this.trySimple = function(item) {
        if (self.usedBlanknodes.find(item) != -1) {
            return;
        }
        var preds = item.preds;
        var coords = [0, 0];
        for (var p in preds) {
            var pred = preds[p];
            var simple = self.parent.simplify(p);
            if (self.latProperties.indexOf(simple) != -1) {
                coords[0] = pred[0];
            } /* latitude */
            if (self.lonProperties.indexOf(simple) != -1) {
                coords[1] = pred[0];
            } /* longitude */
        }
    ...
    this.getMarker = function(item) {
        ...
        case OAT.RDFTabsData.MARKER_MODE_EXPLICIT:
            ...
            if (typeof(mpred) != 'undefined') markerFile = markerPath + mpred + '.png';
            break;
    ...
    this.markerClickHandler = function(caller, msg, m) {
        ...
        if (OAT.AnchorData && OAT.AnchorData.window) {
    ...
    this.makeAndAddMarker = function(x, y, item) {
        ...
        m = self.map.addMarker(x, y, false, {
            image: markerImg,
            imageSize: [18, 41],
            custData: {
                __oat_rdftabs_item: item
            }
        });
    }
    ...
    this.redraw = function() {
        ...
        if (!self.map_loaded) {
            self.map = new OAT.Map(self.elm, self.options.provider, {
                ...
                markerIcon: "toolkit/images/markers/01.png",
                markerIconSize: [18, 41]
            }, self.options.specificOpts);
            OAT.Map.loadApi(self.options.provider, {
                callback: cb
            });
        }
...
OAT.RDFTabs.timeline = function(parent, optObj) {
    ...
    this.bothProperties = ["http://purl.org/dc/elements/1.1/date", "http://purl.org/dc/terms/created", "http://www.w3.org/2002/12/cal/ical#created"]; /* containing date */

    this.startProperties = ["http://microformats.org/wiki/hcalendar/dtstart", "http://www.w3.org/2002/12/cal/ical#dtstart"];

    this.endProperties = ["http://microformats.org/wiki/hcalendar/dtend", "http://www.w3.org/2002/12/cal/ical#dtend"];
    ...
    this.tryDeepItem = function(value) {
        ...
        for (var p in value.preds) {
            ...
            if (self.subProperties.indexOf(p) != -1) {
                return pred[0];
            }
        }
        return false;
    ...
    this.redraw = function() {
        ...
        for (var i = 0; i < self.parent.data.structured.length; i++) {
            ...
            var start = date[0];
            var end = date[1]; /* add event */
            var content = OAT.Dom.create("div", {
                left: "-7px"
            });
            var ball = OAT.Dom.create("div", {
                width: "16px",
                height: "16px",
                cssFloat: "left",
                styleFloat: "left"
...
OAT.RDFTabs.images = function(parent, optObj) {
    ...
    this.showBig = function(index) {
        if (!self.dimmer) {
            ...
            self.container = OAT.Dom.create("div", {
                margin: "auto"
            });
            self.prev = OAT.Dom.create("span", {
                fontWeight: "bold",
                cursor: "pointer"
            });
            self.next = OAT.Dom.create("span", {
                fontWeight: "bold",
                cursor: "pointer"
            });
            ...
            OAT.Event.attach(self.prev, "click", function() {
                self.showBig(self.index - 1);
            });
            OAT.Event.attach(self.next, "click", function() {
                self.showBig(self.index + 1);
            });
        }
        self.index = index;
        ...
        OAT.Event.attach(img, "load", function() {
            ...
            var plus = OAT.Dom.create("strong", {
                cursor: "pointer",
                marginRight: "3px"
            });
            var minus = OAT.Dom.create("strong", {
                cursor: "pointer"
            });
            plus.innerHTML = "+";
    ...
    this.drawThumb = function(index, td) {
        ...
        OAT.Event.attach(img, "click", function() {
            self.showBig(index);
        });
    }
    ...
    this.getImages = function() {
        ...
        for (var i = 0; i < data.length; i++) {
            var item = data[i];
            var preds = item.preds; /* if our uri looks like an image */
            if (self.parent.getContentType(item.uri) == 3) self.addUriItem(item.uri, item); /* FIXME:  perform another regex check ? */
            if (item.uri.match(/http:[^ ]+\.(jpe?g|png|gif)\?.*/gi)) self.addUriItem(item.uri, item);
            for (var p in preds) {
                var pred = preds[p];
                var simple = self.parent.simplify(p);
                ...
                } else { /* look for predicates that are/contain image links */
                    for (var j = 0; j < pred.length; j++) {
                        var value = pred[j];
                        if (typeof(value) == "object") {
                            continue;
...
OAT.RDFTabs.fresnel = function(parent, optObj) {
    ...
    this.redraw = function() {
        var results = self.fresnel.format(self.parent.data.all); /* append stylesheets */
        var ss = results[1];
        ...
        var cb = function(xslDoc) {
                ...
                var find = function(str) {
                        ...
                        for (var i = 0; i < triples.length; i++) {
                            var t = triples[i];
                            if ((t[1] != foaf + "name") && (t[1] != foaf + "knows")) {
                                continue;
                            }
                            if (t[2] == str) {
                                return t;
        ...
        OAT.AJAX.GET(OAT.Preferences.xsltPath + "fresnel2html.xsl", false, cb, {
    ...
    if (self.options.autoload && self.options.defaultURL.length) {
        ...
        var go = function() {
                if ($v(inp)) self.fresnel.addURL($v(inp), self.redraw);
