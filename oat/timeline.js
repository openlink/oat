...
OAT.Timeline = function(contentElm,paramsObj) {
	...
	this.elm = OAT.Dom.create("div",{position:"absolute",top:"0px",left:"0px",class:"timeline"}); /* main axis */
	this.elm.style.zIndex = 3;
	this.content = $(contentElm);
	OAT.Dom.makePosition(self.content);
	
	this.port = OAT.Dom.create("div",{cursor:"w-resize",position:"relative",class:"timeline_port"});
	...
	this.sliderBtn = OAT.Dom.create("div",{class:"timeline_slider"});
	...
	this.draw = function() {
		...
		for (var p in self.bands) {
			...
			var elm = OAT.Dom.create("div",{zIndex:4,position:"absolute",width:"100%",left:"0px",top:(total-1)+"px",textAlign:"center",fontWeight:"bold",class:"timeline_band_header"});
			...
			/* band */
			var elm = OAT.Dom.create("div",{zIndex:1,position:"absolute",left:"0px",width:"100%",class:"timeline_band"});
			if (self.bands[p].color) { elm.style.backgroundColor = self.bands[p].color; }
