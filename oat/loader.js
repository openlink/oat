...
var OAT = {
    ...
    _checkInit: function() {
        if (!this._loaded || !this._winloaded) {
            return;
        }
...
function $$(className, root, tag) {
    ...
    if (OAT.Dom.isClass(e, className)) {
        matches.push(e);
    }
    for (var i = 0; i < elms.length; i++) {
        if (OAT.Dom.isClass(elms[i], className)) {
            matches.push(elms[i]);
        }
    }
    return matches;
...
Array.prototype.find = function(str) {
    for (var i = 0; i < this.length; i++) if (this[i] == str) {
        return i;
    }
    return -1;
}
...
String.prototype.truncate = function(maxlen) {
    var str = this.trim();
    if (!maxlen || maxlen < 2) maxlen = 20;
    if (str.length <= maxlen) return this;
...
Number.prototype.toSize = function() {
    ...
    for (var i = 0; i < post.length; i++) {
        if (result >= stepSize && i + 1 < post.length) {
            ...
            return Math.round(result) + " " + post[i];
        }
    }
    return this;
}
...
OAT.Dom = {
    ...
    imageSrc: function(element, src, srcBlank) {
        ...
        if (png && OAT.Browser.isIE6) {
            if (!srcBlank) {
                srcBlank = OAT.Preferences.imagePath + 'Blank.gif';
            }
            elm.src = srcBlank;
    ...
    hide: function(element) {
        if (arguments.length > 1) {
            for (var i = 0; i < arguments.length; i++) {
                OAT.Dom.hide(arguments[i]);
            }
            return;
        }
        if (element instanceof Array) {
            for (var i = 0; i < element.length; i++) {
                OAT.Dom.hide(element[i]);
            }
            return;
        }
        var elm = $(element);
    ...
    show: function(element) {
        if (arguments.length > 1) {
            for (var i = 0; i < arguments.length; i++) {
                OAT.Dom.show(arguments[i]);
            }
            return;
        }
        if (element instanceof Array) {
            for (var i = 0; i < element.length; i++) {
                OAT.Dom.show(element[i]);
            }
            return;
        }
        var elm = $(element);
        ...
        elm.style.display = ""; /* ie input hack */
        var inputs_ = elm.getElementsByTagName("input");
        var inputs = [];
        for (var i = 0; i < inputs_.length; i++) {
            inputs.push(inputs_[i]);
        ...
        for (var i = 0; i < inputs.length; i++) {
            var inp = inputs[i];
            if (inp.type == "radio" || inp.type == "checkbox") {
                if (inp["__checked"] && inp.__checked === "1") {
                    inp.checked = true;
                }
                if (inp["__checked"] && inp.__checked === "0") {
                    inp.checked = false;
                }
                inp.__checked = false;
            }
    ...
    isChild: function(child, parent) {
        var c_elm = $(child);
        var p_elm = $(parent); /* walk up from the child. if we find parent element, return true */
    ...
    addClass: function(something, className) {
        ...
        if (OAT.Dom.isClass(elm, className)) {
            return;
        }
        var arr = elm.className.split(" ");
        arr.push(className);
    ...
    removeClass: function(something, className) {
        ...
        if (!OAT.Dom.isClass(elm, className)) {
            return;
        } /* cannot remove non-existing class */
        if (className == "*") {
            elm.className = "";
        } /* should not occur */
        var arr = elm.className.split(" ");
    ...
    position: function(something) {
        ...
        if (elm == document.body || elm == document || !parent) {
            return OAT.Dom.getLT(elm);
        }
    ...
    removeSelection: function() {
        var selObj = false;
        if (document.getSelection && !OAT.Browser.isGecko) {
    ...
    changeHref: function(elm, newHref) { /* opera cannot do this with elements not being part of the page :/ */
        var ok = false;
        var e = $(elm);
        var node = e;
...
OAT.Style = {
    ...
    include: function(file, force) {
        ...
        if (!force) { /* prevent loading when already loaded */
            ...
            if (file == styles[i].getAttribute('href') || host + file == styles[i].getAttribute('href')) return;
        }
...
OAT.Browser = {
    ...
    isChrome: function() {
        try {
            if (Components.classes) return true;
            else return false;
        } catch (e) {
            return false;
        }
    ...
    isScreenOnly: ( !! navigator.platform.toString().match(/iphone/i) || !! navigator.platform.toString().match(/symbian/i) || !! navigator.platform.toString().match(/ipod/i) || !! navigator.platform.toString().match(/android/i))
}
...
OAT.Event = {
    ...
    attach: function(elm, event, callback, scope) {
        ...
        if (scope) {
            cb = function() {
                return callback.call(scope, arguments);
            }
        }
...
OAT.MSG = {
    ...
    detach: function(sender, msg, callback) {
        ...
        for (var i = 0; i < OAT.MSG.registry.length; i++) {
            ...
            if (rec[0] == sender && rec[1] == msg && rec[2] == callback) {
                index = i;
            }
        }
        if (index != -1) {
            OAT.MSG.registry.splice(index, 1);
        }
    },
...
OAT.Loader = {
    ...
    _callbacks: {},
    /* array of callbacks for each module */
    _cacheBuster: false,
